...
PassportConfigurator.prototype.configureProvider = function(name, options) {
   ...
   */
  if (authType === 'local') {
    self.app.post(authPath, (options.authHandler ? options.authHandler(passport, options) :
    ...
    self.app.get(authPath, (options.authHandler ? options.authHandler(passport, options) :
